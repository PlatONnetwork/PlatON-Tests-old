...
def create_lockup_plan(client):
    ...
    plan = [{'Epoch': 30, 'Amount': 2 * client.economic.genesis.economicModel.staking.stakeThreshold}]
...
class TestSlashing:
    ...
    #
    #     balance_lockup_1 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
    #     log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
    #                                                                            balance_lockup_1))
    #     create_lockup_plan(clients_noconsensus[0])
    #     balance_lockup_2 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
    #     log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
    #                                                                            balance_lockup_2))
    #
    #     address = pip.node.staking_address
    #     submitvpandvote([clients_noconsensus[0]], votingrounds=2)
    #     balance_lockup_3 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
    #     log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
    #                                                                            balance_lockup_3))
    #
    #     proposalinfo_version = pip.get_effect_proposal_info_of_vote()
    ...
    #     wait_block_number(pip.node, proposalinfo_version.get('EndVotingBlock'))
    #
    #     balance_lockup_4 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
    #     log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
    #                                                                            balance_lockup_4))
    #
    #     submitcppandvote([clients_noconsensus[0]], [1])
    #
    #     balance_lockup_5 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
    #     log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
    #                                                                            balance_lockup_5))
    #
    #     submittpandvote([clients_noconsensus[0]], 1)
    #
    #     balance_lockup_6 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
    #     log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
    #                                                                            balance_lockup_6))
    #
    #     proposalinfo_param = pip.get_effect_proposal_info_of_vote(pip.cfg.param_proposal)
    ...
    #     self.verify_amount_block(pip_test, address, amount, value=6)
    ...
    def test_UNS_PU_020(self, new_genesis_env, clients_noconsensus):
        ...
        create_lockup_plan(clients_noconsensus[0])
        # balance_before = pip.node.eth.getBalance(address)
        # log.info('Block bumber {} staking address balance {}'.format(pip.node.block_number,
        #                                                              balance_before))
        balance_lockup_2 = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS)
        log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format(pip.node.block_number,
                                                                               balance_lockup_2))
        ...
        slashing_amount = block_reward * genesis.economicModel.slashing.slashBlocksReward
        amount = shares - slashing_amount
        log.info(f'处罚金额：{slashing_amount}， 处罚后剩余质押金额：{amount}')
        self.verify_amount_block(pip_test, address, amount, value=5, tag=True)
    ...
    def verify_amount_block(self, pip, address, shares, value, tag=True):
        ...
        wait_block_number(pip.node, (value+1) * pip.economic.settlement_size)
        balance_after = pip.node.eth.getBalance(address, value * pip.economic.settlement_size)
        balance_after_lockup = pip.node.eth.getBalance(pip.economic.cfg.FOUNDATION_LOCKUP_ADDRESS,
                                                       (value+1) * pip.economic.settlement_size)
        log.info('Block bumber {} staking address balance {}'.format(value * pip.economic.settlement_size,
                                                                     balance_after))
        log.info('Block bumber {} FOUNDATION_LOCKUP_ADDRESS balance {}'.format((value+1) * pip.economic.settlement_size,
                                                                               balance_after_lockup))
        assert balance_after == balance_before
        if tag:
            assert balance_after_lockup - balance_before_lockup == shares
